...
/**
 *  构造函数创建 GM 对象 ,创建实例后通过getResults方法返回一个二维数组,
 ...
 * */
var GM = function(array,numOfBuild,numOfPrediction,method){
     ...
     * */
    GM.prototype.getResults = function(){
        var output = [];
        var original = [];
    ...
    * */
    function restoreData(originalOrder,accumuOrder,n,method){
        var b = BasicCal.getBwithT(accumuOrder);
	 ...
	 * @param accumuOrder 一次累加序列
	 */
    BasicCal.getBwithT=function(accumuOrder){
        var b = bgValue.gm11(accumuOrder);
        var matrix = [];
    ...
    bgValue.gm11WithSimpson = 'GM11(Simpson)';

    /**
     * @param arr 一次累加序列
     * GM11基于梯形公式计算背景值
     * */
    bgValue.gm11=function(arr){
        var result =[];
        for(var i=0;i<arr.length-1;i++){
            result[i] = (arr[i]+arr[i+1])/2;
        }
        result = BasicCal.dateFormat(result);
        return result;
    };
