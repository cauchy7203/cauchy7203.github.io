...
/**
 *  构造函数创建 GMC 对象 ,创建实例后通过getResults方法返回一个二维数组,
 ...
 * */
var GMC = function(array,numOfBuild,numOfPrediction,method){
     ...
     * */
    GMC.prototype.getResults = function(){
        var output = [];
        var original = [];
    ...
    * */
    ...
    BasicCal.getBwithT=function(originalOrder, accuOrder){
        var b = bgValue.gm11WithC(originalOrder, accuOrder);
        var matrix = [];
    ...
    bgValue.gm11WithCotes = 'Cotes';


	bgValue.gm11WithC = function(original, accuOrder){
		var arr = [];
	...
	bgValue.DynaAccuOrder = function(originalOrder, accuOrder, k){
		...
		var result1 =  32*((a*(Math.exp(x*(k-(3.0/4.0))))) + b);
		var result2 = 12*((a*(Math.exp(x*(k-(1.0/2.0))))) + b);
		var result3 = 32*((a*(Math.exp(x*(k-(1.0/4.0))))) + b);
		var result = result1 + result2 + result3;
		return ((7*accuOrder[k]) + (7*accuOrder[k-1]) + result)/90;
	};
