...
/**
 *  构造函数创建 GMS 对象 ,创建实例后通过getResults方法返回一个二维数组,
 ...
 * */
var GMS = function(array,numOfBuild,numOfPrediction,method){
     ...
     * */
    GMS.prototype.getResults = function(){
        var output = [];
        var original = [];
    ...
    * */
    ...
    BasicCal.getBwithT=function(originalOrder, accumuOrder){
        var b = bgValue.gm11WithS(originalOrder, accumuOrder);
        var matrix = [];
    ...
    bgValue.gm11WithSimpson = 'Simpson';
	 ...
	 */
	bgValue.gm11WithS = function(originalOrder, accuOrder){
		var arr = [];
		for(let i = 0; i < originalOrder.length - 1; i++){
			arr[i] = bgValue.DynaAccuOrder(originalOrder, accuOrder, i+1);
		}
		return arr;
	};
	...
	bgValue.DynaAccuOrder = function(originalOrder, accuOrder, k){
		...
		var result = (a*Math.exp(x*((2*(k+1)-3)*0.5)))+ b;
		return (accuOrder[k] + accuOrder[k-1] + (4*result))/6;
	};
