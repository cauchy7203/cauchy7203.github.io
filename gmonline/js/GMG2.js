...
/**
 *  构造函数创建 GMG2 对象 ,创建实例后通过getResults方法返回一个二维数组,
 ...
 * */
var GMG2 = function(array,numOfBuild,numOfPrediction,method){
     ...
     * */
    GMG2.prototype.getResults = function(){
        var output = [];
        var original = [];
     ...
     * @return 时间响应序列
    * */
    function restoreData(originalOrder, accuOrder, n,method){
        var b = BasicCal.getBwithT(originalOrder, accuOrder);
	 ...
	 * @param accumuOrder 一次累加序列
	 */
    BasicCal.getBwithT=function(originalOrder, accuOrder){
        var b = bgValue.gm11WithG2(originalOrder, accuOrder);
        var matrix = [];
	 ...
	 */
	bgValue.gm11WithG2 = function(original, accuOrder){
		var arr = [];
		for(let i = 0; i < original.length - 1; i++){
			arr[i] = bgValue.DynaAccuOrder(original, accuOrder, i+1);
		}
		return arr;
	};
	...
	bgValue.DynaAccuOrder = function(originalOrder, accuOrder, k){
		...
		var temp1 = (1/(2*Math.sqrt(3))) + (k-1+0.5);
		var temp2 = (1/(-2*Math.sqrt(3))) + (k-1+0.5);
		
		var result1 = (a*Math.exp(x*temp1)) + b;
		var result2 = (a*Math.exp(x*temp2)) + b;
		var result = (result1 + result2)/2;
		return result;
	};
