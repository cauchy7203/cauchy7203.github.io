...
/**
 *  构造函数创建 GMNC3 对象 ,创建实例后通过getResults方法返回一个二维数组,
 ...
 * */
var GMNC3 = function(array,numOfBuild,numOfPrediction,method){
     ...
     * */
    GMNC3.prototype.getResults = function(){
        var output = [];
        var original = [];
    ...
    * */
    ...
    BasicCal.getBwithT=function(originalOrder, accuOrder){
        var b = bgValue.gm11WithNC3(originalOrder, accuOrder);
        var matrix = [];
    ...
    bgValue.gm11WithNewtonCotes3 = 'NewtonCotes3';


	bgValue.gm11WithNC3 = function(original, accuOrder){
		var arr = [];
	...
	bgValue.DynaAccuOrder = function(originalOrder, accuOrder, k){
		...
		var result1 =  (a*(Math.exp(x*(k-(2.0/3.0))))) + b;
		var result2 = (a*(Math.exp(x*(k-(1.0/3.0))))) + b;
		return (accuOrder[k] + accuOrder[k-1] + 3*(result1+result2))/8;
	};
